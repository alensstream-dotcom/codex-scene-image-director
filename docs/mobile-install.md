...
- 世界书直链：`https://github.com/alensstream-dotcom/codex-scene-image-director/releases/download/v1.5.3/JANIMA_v8_2_Galgame_Director.json`

必须另外安装智绘姬 `st-chatu8`，因为真正的生成图片按钮由智绘姬创建；本插件负责后台纠错、过滤和原位布局，不能代替智绘姬连接 ComfyUI。推荐同时导入正则：`https://github.com/alensstream-dotcom/codex-scene-image-director/releases/download/v1.5.3/JANIMA_rescue_regex.json`，避免历史 Prompt 和 IMG_COUNT 回灌给模型。
