...
本地纠错处理标点、空白、标签去重、`1girl → solo`、`female focus`、缺失构图词、无新剧情的连续按钮和完全重复 Prompt。纯男/纯场景、人数冲突、身份锚点缺失或关键道具错误才通过 `generateQuietPrompt` 修复；修复与补图都获得“上一张图片后到插入点”的完整剧情。默认关闭逐图 AI 深度重审，正常 Prompt 不增加等待时间。
