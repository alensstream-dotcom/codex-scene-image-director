...
2. 导入 `worldbooks/JANIMA_v8_3_Galgame_Director.json`。
3. 关闭旧 v7.8.2、v7.9、v8.1、v8.2 主世界书，避免两套主生图合同同时触发；已有 FM_DNA 变量数据保留。
...
7. 默认启用整轮事件分组、重复动作合并、前后半段覆盖检查、女性门禁和剧情窗口检查，关闭慢速逐图 AI 深度重审；正常 Prompt 会直接变成按钮，分布错误时才整轮重排。
