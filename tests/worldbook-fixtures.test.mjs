...
test('v8.2 mobile worldbook enforces segment-aware female Galgame shots and identity locks', async () => {
    ...
    assert.match(worldbook.entries['2'].content, /\{\{getvar::stat_data\}\}/);
    assert.doesNotMatch(JSON.stringify(worldbook), /<%[_=]?/);
    assert.match(worldbook.entries['2'].content, /不依赖 EJS、酒馆助手或 JS-Slash-Runner/);
    assert.match(worldbook.entries['3'].content, /FM_DNA \/ FM_SCENE \/ FM_ANCHOR/);
    assert.match(worldbook.entries['4'].comment, /v8\.2/);
    assert.equal(worldbook.entries['4'].disable, false);
    assert.equal(worldbook.entries['4'].order, 999);
    assert.match(worldbook.entries['4'].content, /正常含女性的剧情目标\s*3\s*张/);
    assert.match(worldbook.entries['4'].content, /硬上限\s*6\s*张/);
    assert.match(worldbook.entries['4'].content, /上一张 Prompt 之后/);
    assert.match(worldbook.entries['4'].content, /窗口内全部剧情再选镜/);
    assert.match(worldbook.entries['4'].content, /女性硬门禁/);
    assert.match(worldbook.entries['4'].content, /单独男性、纯场景、纯建筑、纯道具/);
    assert.match(worldbook.entries['4'].content, /整轮完全没有真实可见女性[\s\S]*IMG_COUNT:0/);
    assert.match(worldbook.entries['4'].content, /最佳镜头赢家规则/);
    assert.match(worldbook.entries['4'].content, /禁止\s*\[Unnamed Persona\]/);
    assert.match(worldbook.entries['4'].content, /<!--IMG_COUNT:n-->/);
    assert.match(worldbook.entries['2'].content, /CANONICAL LOCK: Lucifer/);
    assert.match(worldbook.entries['2'].content, /CANONICAL LOCK: Leviathan/);
    assert.match(worldbook.entries['2'].content, /CANONICAL PROP LOCK: Behemoth/);
    assert.match(worldbook.entries['4'].content, /two separate bodies/);
    assert.match(worldbook.entries['4'].content, /角色名字不是外貌/);
    assert.match(worldbook.entries['4'].content, /gothic dress with water-pattern trim:1\.25/);
    assert.match(worldbook.entries['4'].content, /bodysuit、leotard、lingerie/);
    assert.match(worldbook.entries['4'].content, /masterpiece, best quality, score_7, highres, newest/);
    assert.match(worldbook.entries['4'].content, /普通内容 safe；擦边\/内衣 sensitive；裸露 nsfw；明确性行为 explicit/);
    assert.match(worldbook.entries['4'].content, /成人剧情照常出图/);
    assert.match(worldbook.entries['4'].content, /不得仅因内容属于 NSFW 就省略 Prompt/);
    assert.match(worldbook.entries['4'].content, /正常目标 3 张[\s\S]*4–6 张/);
    assert.match(worldbook.entries['4'].content, /裸体镜头也不会换人/);
    assert.match(worldbook.entries['4'].content, /只允许一个以 @ 开头的精确画师标签/);
    assert.match(worldbook.entries['2'].content, /PREVIOUS SHOT CONTINUITY ANCHOR/);
    assert.match(worldbook.entries['2'].content, /ANIMA STYLE ANCHOR/);
});
...
test('runtime is silent and uses only the verified inline Zhihuiji button route', async () => {
    ...
    assert.match(source, /native payload cannot be proven to match/);
