...
test('runtime is silent and uses only the verified inline Zhihuiji button route', async () => {
    ...
    assert.match(source, /streamingEvidenceSettles: new Map/);
    assert.match(source, /scheduleCheck\(settleId, 650\)/);
    assert.match(source, /streamCompletionTimer: null/);
    assert.match(source, /A quiet token window is the/);
    assert.match(source, /setInterval\(ensureLatestAssistantStoryboard, 1200\)/);
    assert.match(source, /JANIMA_SCENE_IMAGE_DIRECTOR/);
    assert.match(source, /const sourceShots = sourceLedger\?\.payload\?\.shots \|\| \[\]/);
    assert.doesNotMatch(source, /ledger\.payload\.shots/);
