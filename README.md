...
```text
主模型 + JANIMA v8.7 手机 Galgame 纯剧情世界书 → 回复完成后一次批量全文分镜 → 插件动作/女性门禁/DNA 校验 → 智绘姬真实原位按钮 → JANIMA Galgame Turbo 8步工作流
```
...
1. 手机下载并导入 `worldbooks/JANIMA_v8_7_Galgame_PostReply_Director.json`。它保留 FM_DNA、场景变量与 UpdateVariable，并使用 SillyTavern 原生变量宏，不依赖 EJS/JS-Slash-Runner。
2. 关闭旧 v7.8.2、v7.9、v8.1、v8.2、v8.3、v8.4、v8.5、v8.6 主生图世界书，避免多套规则同时生效；已有 FM_DNA 变量数据不用删除。
...
- v8.7 让主回复只写自然剧情；回复完成后插件只调用当前聊天模型一次，读取整轮正文并同时生成 1–6 张 Prompt。不是每张图调用一次，也不使用智绘姬内部扩写。
- 本地事件账本先合并持续动作并分配开场/中段/结尾窗口；后台批量模型只能在这些真实窗口中选最精彩帧。返回动作必须与窗口动作类型重合，否则整轮结果被拒绝并快速重试。
...
- v8.6 的同次证据包仍兼容，但“主回复同步写 Prompt”实验开关默认关闭；外部 API 审计和逐图二次改写也默认关闭。
...
- 相同人物从 FM_DNA、人物卡和最近内联 Prompt 沿用不可变外貌与未变化服装；只有正文证明换装、脱衣或永久变化时才允许改变。
