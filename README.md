...
```text
主模型 + JANIMA v8.6 手机 Galgame 剧情证据世界书 → 同次回复内的原文证据包与流式 Prompt → 插件原句/动作/DNA 校验 → 智绘姬真实按钮 → JANIMA Galgame Turbo 8步工作流
```
...
1. 手机下载并导入 `worldbooks/JANIMA_v8_6_Galgame_Evidence_Director.json`。它保留 FM_DNA、场景变量与 UpdateVariable，并使用 SillyTavern 原生变量宏，不依赖 EJS/JS-Slash-Runner。
2. 关闭旧 v7.8.2、v7.9、v8.1、v8.2、v8.3、v8.4、v8.5 主生图世界书，避免多套规则同时生效；已有 FM_DNA 变量数据不用删除。
3. 导入 `regex/JANIMA_rescue_regex.json`。
4. 智绘姬标记设置为 `[` 和 `]`，关闭 LLM 扩写、正文二次分析、自动重写和自动风格。

## 后台行为

- 静默检查最近 16 条含剧情证据的 assistant 回复，不向 `.mes` 追加任何可见插件节点；旧消息滚回手机视口时会重新核验，避免聊多层后按钮消失。
...
- v8.6 在当前剧情回复中先写逐镜头 `JANIMA_SHOT` 原文证据包，再立刻写对应 Prompt；插件监听方括号闭合并立即唤醒智绘姬。若模型漏方括号或写错顺序，插件只从同次回复已经给出的原文证据补格式，绝不再用通用动作/场景模板猜图，也不默认发起第二次 LLM。
...
- 每张 Prompt 前的隐藏证据包必须逐字引用上方正文；引用未来文本、动作阶段不一致或单男性证据会被拒绝。
- 相同 `cast.id` 的不可变 DNA 跨回合锁定；服装只有在引用原文本身证明换装/脱衣时才允许变化。
