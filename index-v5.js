...
import { clearAnchorArtifacts, insertSlotAtExactAnchor } from './lib/dom-anchor-v5.mjs';
import {
...
const DEFAULT_SETTINGS = Object.freeze({
    ...
    minimumShots: 3,
    maximumShots: 5,
    stateResponseLength: 2400,
    directorResponseLength: 2400,
...
function settings() {
    ...
    const legacyLength = Number(existing.plannerResponseLength) || 0;
    if (!Number(existing.stateResponseLength) && legacyLength) next.stateResponseLength = legacyLength;
    if (!Number(existing.directorResponseLength) && legacyLength) next.directorResponseLength = legacyLength;
    ...
    next.stateResponseLength = Math.max(1000, Math.min(6000, Number(next.stateResponseLength) || 2400));
    next.directorResponseLength = Math.max(1000, Math.min(6000, Number(next.directorResponseLength) || 2400));
    ...
    if (!String(next.fixedPositive || '').trim()) next.fixedPositive = DEFAULT_V5_POSITIVE;
    if (!String(next.fixedNegative || '').trim()) next.fixedNegative = DEFAULT_V5_NEGATIVE;
...
function currentCharacterContext() {
    ...
    return {
        ...
        visual: [character.description, character.data?.description]
            .filter(Boolean).join(' ').replace(/\s+/g, ' ').slice(0, 2600),
    };
}
...
function latestAssistantId() {
    for (let index = (chat?.length || 0) - 1; index >= 0; index--) if (isAssistantMessage(index)) return index;
    return -1;
}
...
function cleanStory(raw = '') {
    return stripLegacyImagePromptLines(extractNarrativeStory(stripProtocol(raw)))
        .replace(/<!--\s*JANIMA_(?:V3|V4|V5)[\s\S]*?-->/gi, '')
        .trim();
}
...
function storageFor(message, create = false) {
    ...
    if (!message.extra[STORAGE_KEY] && create) {
        message.extra[STORAGE_KEY] = { version: 10, processedHash: '', state: null, director: null, visualMemory: {}, shots: [] };
    }
    const storage = message.extra[STORAGE_KEY] || null;
    if (storage && create) {
        storage.version = 10;
        storage.processedHash ||= '';
        storage.visualMemory = storage.visualMemory && typeof storage.visualMemory === 'object' ? storage.visualMemory : {};
...
function priorVisualMemory(limit = 30) {
    const memory = {};
    for (let messageId = Math.max(0, (chat?.length || 0) - limit); messageId < (chat?.length || 0); messageId++) {
        const stored = storageFor(chat[messageId]);
        for (const [name, entry] of Object.entries(stored?.visualMemory || {})) {
            memory[name] = {
                identity: String(entry.identity || memory[name]?.identity || '').slice(0, 1200),
                outfit: String(entry.outfit || memory[name]?.outfit || '').slice(0, 900),
            };
        }
    }
    return memory;
...
function beginGeneration(type, options = {}, dryRun = false) {
    if (runtime.internalCall || dryRun || !settings().enabled || !settings().automatic) return;
...
async function runJsonStage({ request, responseLength, retry, parser, label }) {
    runtime.internalCall = true;
    try {
        ...
        try {
            return { parsed: parser(raw), raw, retried: false };
        } catch (error) {
            if (!retry) throw error;
            const repaired = await generateRaw({
                prompt: `${request.userPrompt}\n\nThe previous ${label} output failed validation. Return corrected strict JSON only. Previous output:\n${String(raw || '').slice(0, 6000)}`,
                systemPrompt: `${request.systemPrompt}\nThe previous output was invalid. Correct JSON structure and preserve only facts supported by the supplied paragraphs.`,
                responseLength,
            });
            return { parsed: parser(repaired), raw: repaired, retried: true };
        }
    } finally {
        runtime.internalCall = false;
    }
}
...
async function processAssistantMessage(messageId, { messageType = '', force = false } = {}) {
    ...
    try {
        ...
        storage.shots = [];
        storage.state = null;
        storage.director = null;

        const config = settings();
        const memory = priorVisualMemory();
        let stateRun = { parsed: { characters: [], events: [], errors: ['state extractor unavailable'] }, raw: '', retried: false };
        try {
            const request = buildStateExtractorPrompt({ story, card: currentCharacterContext(), priorMemory: memory });
            stateRun = await runJsonStage({
                request,
                responseLength: config.stateResponseLength,
                retry: config.stateRetry,
                parser: raw => parseStateResponse(raw, story),
                label: 'state extractor',
            });
        } catch (error) {
            console.error(`[${EXT_NAME}] 状态提取失败`, error);
            toast('warning', `状态提取失败，将使用保守状态：${error?.message || error}`);
        }
        const timeline = buildStateTimeline(story, stateRun.parsed, memory);

        let directorRun = { parsed: { targetCount: config.minimumShots, shots: [], errors: ['director unavailable'] }, raw: '', retried: false };
        try {
            const request = buildDirectorPromptV5({ story, timeline, minimum: config.minimumShots, maximum: config.maximumShots });
            directorRun = await runJsonStage({
                request,
                responseLength: config.directorResponseLength,
                retry: config.directorRetry,
                parser: raw => parseDirectorResponse(raw, story, timeline),
                label: 'storyboard director',
            });
        } catch (error) {
            console.error(`[${EXT_NAME}] 镜头导演失败`, error);
            toast('warning', `镜头导演失败，改用本地候选：${error?.message || error}`);
        }

        const fallback = config.localFallback ? buildFallbackCandidatesV5(story, timeline) : [];
        const plan = selectShotsV5({
            story,
            directorShots: directorRun.parsed.shots,
            fallbackShots: fallback,
            minimum: config.minimumShots,
            maximum: config.maximumShots,
            requestedCount: directorRun.parsed.targetCount,
        });

        storage.state = {
            retried: stateRun.retried,
            errors: stateRun.parsed.errors,
            characterCount: timeline.registry.length,
        };
        storage.director = {
            retried: directorRun.retried,
            errors: directorRun.parsed.errors,
            diagnostics: plan.diagnostics,
        };
        storage.visualMemory = timeline.memory;

        const generationId = `v5-${id}-${storyHash}`;
        const records = [];
        for (const shot of plan.shots) {
            const evidence = `${shot.paragraph} ${(shot.state?.characters || []).map(item => `${item.name} ${item.identity}`).join(' ')}`;
            if (shot.nsfwLevel > 0 && MINOR_RE.test(evidence)) continue;
            const compiled = composePromptV5(shot, {
            ...
            const record = {
                ...
                anchorSpec: shot.anchorSpec,
                quote: shot.paragraph,
                shot,
                prompt: compiled.positive,
                negative: compiled.negative,
                seed: seedForShotV5(shot),
                retry: 0,
                status: 'queued',
                promptId: '',
                url: '',
        ...
        console.info(`[${EXT_NAME}] V5 stateful storyboard`, {
            stateCharacters: timeline.registry.length,
            directorCandidates: directorRun.parsed.shots.length,
            fallbackCandidates: fallback.length,
            selected: records.length,
            diagnostics: plan.diagnostics,
            stateErrors: stateRun.parsed.errors,
            directorErrors: directorRun.parsed.errors,
        });
        if (!records.length) toast('error', '没有找到可用镜头，请展开导演详情查看原因。');
    } catch (error) {
        console.error(`[${EXT_NAME}] V5 处理失败`, error);
...
function recordsForMessage(messageId) {
    ...
    for (const record of runtime.jobs.values()) if (Number(record.messageId) === Number(messageId)) map.set(recordKey(record), record);
...
function buildPlanningSlot() {
    ...
    text.textContent = 'V5 正在先冻结人物、服装和地点状态，再从全文选择最强镜头…';
...
function sceneDetails(record) {
    ...
    summary.textContent = `V5 详情 · ${record.shot?.source === 'director' ? '独立导演' : '本地补漏'} · P${record.paragraphIndex} · NSFW ${record.shot?.nsfwLevel || 0} · ${record.shot?.stage || 'scene'}`;
    const quote = document.createElement('div');
    quote.className = 'janima-autocg-quote';
    quote.textContent = `取材：${record.quote || ''}`;
    const state = document.createElement('div');
    state.className = 'janima-autocg-meta';
    const characters = (record.shot?.state?.characters || []).map(item => `${item.name}[${item.outfit || '未明确服装'}]`).join('；');
    state.textContent = `冻结状态：${characters || '未识别角色'}；地点=${record.shot?.state?.location || '未明确'}；入选理由=${record.shot?.reason || '本地规则'}`;
    const prompt = document.createElement('code');
    prompt.className = 'janima-autocg-prompt';
    prompt.textContent = `Positive：${record.prompt}\n\nNegative：${record.negative}`;
    details.append(summary, quote, state, prompt);
    return details;
}
...
function renderMessage(messageId) {
    ...
    clearAnchorArtifacts(root);
    const previousByKey = new Map();
    const failed = [];
    for (const record of recordsForMessage(messageId)) {
        const slot = buildSlot(record);
        const result = insertSlotAtExactAnchor(root, slot, record.anchorSpec || { quote: record.quote }, previousByKey);
        if (!result.inserted) {
            slot.classList.add('is-anchor-error');
            const warning = document.createElement('div');
            warning.className = 'janima-autocg-status';
            warning.textContent = '精确剧情锚点未找到，因此没有把图片乱插到别处。';
    ...
    if (failed.length) {
        ...
        errors.className = 'janima-autocg-anchor-errors';
        const title = document.createElement('div');
        title.className = 'janima-autocg-status';
        title.textContent = '以下CG定位失败（保留在消息末尾供检查）：';
        errors.append(title, ...failed);
        root.append(errors);
    }
...
function friendlyError(error) {
    ...
    if (/Failed to fetch|NetworkError|Load failed/i.test(text)) return '无法直连 ComfyUI。确认电脑局域网 IP、--listen 0.0.0.0 和 --enable-cors-header *。';
    return text;
}
...
async function generateRecord(record) {
    ...
    try {
        ...
        if (!record.promptId) {
            ...
            const submitted = await submitComfyPrompt(config.comfyUrl, workflow, { signal: record.controller.signal, timeoutMs: 15000 });
...
function bindSlotActions() {
    document.addEventListener('click', event => {
        ...
        if (target.dataset.action === 'reroll') {
            ...
            record.seed = seedForShotV5(record.shot || {}, record.retry);
            record.promptId = '';
            record.url = '';
            record.image = null;
            record.error = '';
...
function settingsHtml() {
    return `<div id="janima_autocg_settings" class="extension_container janima-autocg-settings"><div class="inline-drawer">
      ...
      <div class="inline-drawer-content">
        ...
        <label class="checkbox_label"><input type="checkbox" name="enabled">启用 V5 状态化导演</label>
        <label class="checkbox_label"><input type="checkbox" name="stateRetry">状态 JSON 无效时自动修复一次</label>
        <label class="checkbox_label"><input type="checkbox" name="directorRetry">镜头 JSON 无效时自动修复一次</label>
        <label class="checkbox_label"><input type="checkbox" name="localFallback">模型漏掉关键动作/成人阶段时本地补漏</label>
        <label class="checkbox_label"><input type="checkbox" name="showSceneDetails">显示冻结服装、地点、入选理由和最终 Prompt</label>
        <small>V5 使用两次隔离分析：先冻结每段的人物、当前服装和地点，再从全文用质量与差异性算法选择3–5张。图片通过渲染文字字符锚点定位；找不到时会明确报错，不再随便堆到中间或末尾。</small>
        ...
        <div class="janima-autocg-settings-row"><label>状态提取 Tokens<input class="text_pole" name="stateResponseLength" type="number" min="1000" max="6000" step="100"></label><label>镜头导演 Tokens<input class="text_pole" name="directorResponseLength" type="number" min="1000" max="6000" step="100"></label></div>
        <label>人物强制描述（全局，通常留空）<textarea class="text_pole" name="manualCharacterLock" rows="3"></textarea></label>
        <label>服装强制描述（全局，通常留空）<textarea class="text_pole" name="manualOutfitLock" rows="3"></textarea></label>
...
function bindEvents() {
    ...
    if (event_types.MESSAGE_RECEIVED) eventSource.on(event_types.MESSAGE_RECEIVED, (messageId, messageType) => setTimeout(() => void processAssistantMessage(messageId, { messageType }), 100));
    if (event_types.GENERATION_ENDED) {
        eventSource.on(event_types.GENERATION_ENDED, () => {
            if (runtime.internalCall) return;
            const id = latestAssistantId();
            if (id >= 0) setTimeout(() => void processAssistantMessage(id), 140);
        });
    }
    if (event_types.MESSAGE_SWIPED) eventSource.on(event_types.MESSAGE_SWIPED, messageId => setTimeout(() => void processAssistantMessage(messageId, { force: true }), 120));
...
function observeChat() {
    ...
    runtime.observer = new MutationObserver(mutations => {
        const meaningful = mutations.some(mutation => [...mutation.addedNodes, ...mutation.removedNodes]
            .some(node => !(node instanceof Element) || !node.matches('.janima-autocg-slot, .janima-autocg-anchor-marker')));
        if (meaningful) scheduleRender(100);
    });
...
jQuery(() => {
    ...
    globalThis.JANIMA_AUTO_CG = Object.freeze({
        ...
        inspectLatest: () => {
            const id = latestAssistantId();
            const storage = id >= 0 ? storageFor(chat[id]) : null;
            return {
                state: storage?.state,
                director: storage?.director,
                visualMemory: storage?.visualMemory,
                shots: id >= 0 ? recordsForMessage(id).map(record => ({
                    paragraphIndex: record.paragraphIndex,
                    quote: record.quote,
                    source: record.shot?.source,
                    reason: record.shot?.reason,
                    stage: record.shot?.stage,
                    nsfwLevel: record.shot?.nsfwLevel,
                    characters: record.shot?.state?.characters,
                    location: record.shot?.state?.location,
                    prompt: record.prompt,
                    negative: record.negative,
                    status: record.status,
                })) : [],
        ...
        state: () => ({ queue: runtime.queue.length, running: runtime.running ? recordKey(runtime.running) : null, settings: settings() }),
    });
    document.documentElement.dataset.janimaAutocgVersion = VERSION;
    console.info(`[${EXT_NAME}] v${VERSION} loaded; stateful two-pass director`);
});
