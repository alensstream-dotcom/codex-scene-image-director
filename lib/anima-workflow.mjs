...
export function installAnimaAccuracyWorkflow(chatu8Settings) {
    ...
    let changed = chatu8Settings.workers[ANIMA_ACCURACY_WORKFLOW_ID] !== workflow
        || !(chatu8Settings.scriptEnabled === true || chatu8Settings.scriptEnabled === 'true')
        || chatu8Settings.mode !== 'comfyui'
        || chatu8Settings.startTag !== '['
        || chatu8Settings.endTag !== ']'
    ...
    // A clean phone install frequently leaves Chatu8's scanner disabled. The
    // director cannot create native buttons unless this switch and delimiters
    // are enabled, so make the mobile contract self-configuring.
    chatu8Settings.scriptEnabled = true;
    chatu8Settings.mode = 'comfyui';
    chatu8Settings.startTag = '[';
    chatu8Settings.endTag = ']';
    chatu8Settings.workerid = ANIMA_ACCURACY_WORKFLOW_ID;
