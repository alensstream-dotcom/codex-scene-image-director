export const ANIMA_ACCURACY_WORKFLOW_ID = 'JANIMA_剧情准确_30步_角色锁_v1';
...
/**
 * Text-only accuracy workflow for JANIMA/Anima checkpoints.
 *
 * JANIMA is an Anima transformer checkpoint, so classic SD1.5/SDXL IP-Adapter
 * graphs are not compatible. This workflow instead follows Anima Base's
 * high-guidance sampling range and relies on complete per-shot visual DNA.
 */
export function buildAnimaAccuracyWorkflow() {
    return JSON.stringify({
        ...
        13: {
            inputs: { filename_prefix: 'ST_JANIMA_ACCURATE', images: ['12', 0] },
...
export function installAnimaAccuracyWorkflow(chatu8Settings) {
    ...
    let changed = chatu8Settings.workers[ANIMA_ACCURACY_WORKFLOW_ID] !== workflow
        ...
        || Number(chatu8Settings.comfyui_steps) !== 30
        || Number(chatu8Settings.cfg_comfyui) !== 4
        || chatu8Settings.comfyuisamplerName !== 'er_sde'
        || chatu8Settings.comfyui_scheduler !== 'simple';
    chatu8Settings.workers[ANIMA_ACCURACY_WORKFLOW_ID] = workflow;
    chatu8Settings.workerid = ANIMA_ACCURACY_WORKFLOW_ID;
    chatu8Settings.worker = workflow;
    chatu8Settings.comfyui_steps = 30;
    chatu8Settings.cfg_comfyui = 4;
    chatu8Settings.comfyuisamplerName = 'er_sde';
    chatu8Settings.comfyui_scheduler = 'simple';
