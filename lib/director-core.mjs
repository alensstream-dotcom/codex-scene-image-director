...
function weightedAppearanceTags(value = '') {
    ...
    add(/(?<!银)白色?(?:长发|短发|头发|马尾)|white hair/i, 'strikingly consistent white hair');
...
function englishSceneTags(value = '') {
    ...
    add(/咖啡|coffee/i, 'steaming coffee cup clearly visible in her hands');
    add(/(?:递|推|端)[^。\n]{0,18}(?:咖啡|杯)|hand(?:ing|s)?[^,.]{0,18}(?:coffee|cup)|offer(?:ing|s)?[^,.]{0,18}(?:coffee|cup)/i, "heroine handing a steaming coffee cup directly to her male companion, cup and both hands centered in frame");
...
export function resolveOutfitFromStory(storyPrefix = '', carriedOutfit = '', modelCandidate = '') {
    ...
    for (const sentence of storySentences(storyPrefix)) {
        ...
        if (explicitNude) {
            ...
            const genericUniform = next === 'school uniform' && /uniform|校服|制服/i.test(outfit);
            if (genericUniform && !wearsNew) {
                // Preserve a more specific carried uniform (colour, jacket,
                // skirt, tie) when the story only says "the uniform".
        ...
        if (/湿透|淋湿|湿漉|soaked|wet clothes/i.test(sentence.text) && outfit && !/\bwet\b/i.test(outfit)) {
            outfit = `wet ${outfit}`;
            changed = true;
        }
...
export function groundedActionFromEvidence(evidence = '', candidate = '') {
    const source = normalizedText(evidence);
    const stage = fallbackStage(source);
    const canonical = {
...
export function fallbackStage(text = '') {
    ...
    if (/(?:递|推|端)[^。\n]{0,24}(?:咖啡|杯)|(?:咖啡|杯)[^。\n]{0,24}(?:递|交|送|塞|放进)|hand(?:ing)?[^,.]{0,24}(?:coffee|cup)|offer(?:ing)?[^,.]{0,24}(?:coffee|cup)/i.test(text)) return 'coffee_handoff';
...
export function buildFallbackPackets(story, { bible = createBible(), characterName = '', characterVisual = '', minimum = 1, maximum = 2 } = {}) {
    ...
    return selected.map((item, index) => {
        ...
        const grounded = groundedActionFromEvidence(storyWindow, '');
        ...
        return sanitizePacket({
            ...
            setting: `Use only the location, weather, clothing and props established in this verbatim story window: ${storyWindow}`,
