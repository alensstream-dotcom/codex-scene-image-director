...
function weightedAppearanceTags(value = '') {
    ...
    add(/紫(?:色)?(?:眼睛|眼眸|瞳孔|瞳)|purple eyes/i, 'striking purple eyes');
...
const SCENE_CONCEPTS = Object.freeze([
    ['object_slam', /砰地|重重(?:放|拍|摔|砸)|猛地(?:放|拍|摔|砸)|slam|smash|pound/i],
    ['group_reaction', /目光[^。\n]{0,30}(?:来回|移动|扫过)|暴怒[^。\n]{0,30}焦急|group[_ -]?(?:reaction|tension)|conflicting emotions|tense tableau/i],
    ...
    ['sword', /剑|刀|刃|斩|砍|刺(?:入|中|向|穿|伤|出|击|杀|去|来)|格挡|sword|blade|slash|stab|block/i],
...
export function sceneSettingFromStory(storyPrefix = '') {
    let latest = '';
    // Strip dialogue before sentence splitting. Chinese dialogue commonly
    // contains a full stop, which otherwise separates the opening and closing
    // quote and lets a spoken destination leak back into location detection.
    const narrationPrefix = String(storyPrefix || '')
        .replace(/[“「『][\s\S]*?[”」』]/g, ' ')
        .replace(/"[\s\S]*?"/g, ' ');
    for (const sentence of storySentences(narrationPrefix)) {
        // "I must go to school" spoken in a living room must never turn the
        // current breakfast CG into an outdoor school image.
        const narration = sentence.text.trim();
        if (LOCATION_EVIDENCE_RE.test(narration)
            || /家中|家里|餐厅|餐桌|饭桌|玄关|厨房|客厅|living room|dining room|breakfast table|entryway|kitchen/i.test(narration)) {
            latest = narration;
        }
    }
    return latest;
...
function identityTags(value = '') {
    ...
    add(/短发|short hair/i, 'short hair');
    add(/齐刘海|平刘海|straight bangs|blunt bangs/i, 'straight bangs');
    add(/紫(?:色)?(?:眼睛|眼眸|瞳孔|瞳)|purple eyes/i, 'purple eyes');
    ...
    for (const sentence of storySentences(storyPrefix)) {
        ...
        if (explicitNude) {
            ...
            if (hasGenericUniform && specificUniform && !wearsNew) {
                ...
                const additions = directTags.filter(tag => tag !== 'school uniform')
                    .filter(tag => !(tag === 'dark navy uniform' && /dark navy uniform/i.test(outfit)))
                    .filter(tag => !(tag === 'sailor school uniform' && /sailor school uniform/i.test(outfit)));
                outfit = uniqueTags([outfit, additions]).join(', ');
...
export function compilePrompt(packet, bible, extraDna = '') {
    ...
    const positive = uniqueTags([
        // Keep the ownership marker and grounding evidence before any long
        // appearance expansion. ChatU8 caps an inline prompt at 1800 chars.
        QUALITY_TAGS,
        'codex galgame director grounded prompt',
        shot.safety,
        shot.people,
        castTags,
        shot.action ? `PRIMARY VISUAL SUBJECT, highest priority, must dominate the frame: ${cleanField(shot.action, 360)}` : '',
...
export function fallbackStage(text = '') {
    ...
    // A bare "刺" also occurs in non-combat words such as "刺眼". Require a
    // visible stabbing verb/result so bright sunlight cannot become a fight.
    if (/打击|打斗|挥打|击打|打中|砍|刺(?:入|中|向|穿|伤|出|击|杀|去|来)|射|剑|利爪|拔刀|拔剑|挡住|格挡|斩|劈|fight|strike|block|draw sword/i.test(text)) return 'combat';
