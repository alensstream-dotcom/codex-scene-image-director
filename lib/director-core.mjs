...
export function compilePrompt(packet, bible, extraDna = '') {
    ...
    const interactionTags = maleScene
        ? [
            '(one adult woman:1.45)', '(one adult man:1.45)', '(adult male companion:1.4)',
            '(short dark hair on male companion:1.3)', '(masculine male face:1.3)',
...
function fallbackStage(text = '') {
    ...
    if (/(?:递|推|端)[^。\n]{0,24}(?:咖啡|杯)|hand(?:ing)?[^,.]{0,24}(?:coffee|cup)|offer(?:ing)?[^,.]{0,24}(?:coffee|cup)/i.test(text)) return 'coffee_handoff';
