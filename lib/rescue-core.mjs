...
const INSTANT_PHASE_TAGS = Object.freeze({
    outfit_change: ['changing clothes', 'clothing in motion', 'revealing new state'],
    erotic_touch: ['erotic touch', 'body-to-body contact', 'woman initiating intimate contact'],
    manual_stimulation: ['manual stimulation', 'woman using her hand', 'visible intimate interaction'],
    oral_sex: ['oral sex', 'woman actively using her mouth', 'visible intimate interaction'],
    penetration: ['vaginal sex', 'initial penetration', 'bodies joined', 'woman reacting to first contact'],
    thrusting: ['vaginal sex', 'continued rhythmic motion', 'same position', 'bodies joined'],
    position_change: ['vaginal sex', 'position change', 'woman taking the active position', 'bodies joined'],
    climax: ['orgasm', 'woman climaxing', 'body trembling', 'intense visible reaction'],
    ejaculation: ['ejaculation', 'visible climax result', 'intense visible reaction'],
    aftercare: ['aftercare', 'resting together', 'close body contact', 'relaxed expression'],
    enter: ['woman entering the scene', 'clothing in motion', 'decisive entrance'],
    leave: ['woman leaving the scene', 'turning away', 'decisive departure'],
    kiss: ['kissing', 'close body contact', 'woman-led interaction'],
    embrace: ['embracing', 'close body contact', 'woman-led interaction'],
    attack: ['woman attacking', 'dynamic action', 'visible impact'],
    defend: ['woman defending', 'dynamic action', 'protective stance'],
    reveal: ['dramatic reveal', 'visible state change', 'surprised expression'],
    default: ['woman performing the decisive story action', 'visible interaction', 'expressive face'],
});
const CONTINUITY_TAG_RE = /(?:\b(?:hair|eyes?|skin|face|build|body|breasts?|hips?|waist|horns?|wings?|tail|ears?|bangs?|braid|ponytail|twintails?|dress|uniform|shirt|skirt|coat|jacket|robe|armor|lingerie|underwear|stockings?|socks?|boots?|shoes?|gloves?|necklace|choker|earrings?|hair ornament|ribbon|bow|crown|tiara)\b|^@)/i;
const TRANSIENT_TAG_RE = /(?:\b(?:standing|sitting|kneeling|lying|running|walking|kissing|embracing|touching|sex|penetration|orgasm|climax|aftercare|expression|expressive|flushed|tearful|surprised|angry|smile|trembling|joined|contact|motion|shot|view|angle|framing|composition|lighting|background|room|street|forest|bed|sofa)\b)/i;
const MALE_STORY_RE = /(?:\b(?:man|male|boy|husband|boyfriend|male protagonist)\b|男人|男性|男主|丈夫|男友|少年|他(?:的|们|正|在|被|把|向|与|和|紧|缓|突|已|又|也))/i;
const MULTI_FEMALE_STORY_RE = /(?:她们|两名?女|两个女孩|二女|姐妹|\b(?:2girls|two women|two girls|women|girls)\b)/i;
const SCENE_TAGS = [
    [/(?:卧室|床上|床边|bedroom|bed)/i, 'bedroom'],
    [/(?:浴室|浴池|温泉|bathroom|bath|hot spring)/i, 'bathroom'],
    [/(?:教室|学校|校园|classroom|school|campus)/i, 'school interior'],
    [/(?:街道|小巷|广场|street|alley|plaza)/i, 'street'],
    [/(?:森林|树林|forest|woods)/i, 'forest'],
    [/(?:洞穴|山洞|cave|cavern)/i, 'cave'],
    [/(?:宫殿|大厅|殿堂|palace|hall)/i, 'grand hall'],
];
...
export function storyBeatGroups(text = '') {
    ...
    const analyzed = candidates.map((item, order) => {
        ...
        const femaleContext = directFemale || order - lastFemaleOrder <= 2;
...
export function planStoryboardSlots(text = '', options = {}) {
    ...
    const { candidates, groups } = storyBeatGroups(text);
...
function instantContinuityTags(previousPrompt = '') {
    return splitTags(previousPrompt)
        .filter(tag => (CONTINUITY_TAG_RE.test(tag) || /^(?:young )?adult woman$/i.test(tag) || /^[A-Z][A-Za-z0-9 .'-]{1,30}$/.test(tag)) && !TRANSIENT_TAG_RE.test(tag))
        .filter(tag => !ANIMA_NATIVE_QUALITY_TAGS.includes(tag.toLowerCase()) && !ANIMA_SAFETY_TAGS.includes(tag.toLowerCase()))
        .slice(0, 12);
}

function instantParticipantTags(windowText = '') {
    if (MULTI_FEMALE_STORY_RE.test(windowText)) return ['2girls', 'female focus', 'separate bodies', 'both faces visible'];
    if (MALE_STORY_RE.test(windowText)) return ['1girl and 1boy', 'female focus', 'male protagonist partly visible'];
    return ['1girl', 'female focus'];
}

function instantPhaseTags(slot = {}) {
    const phase = String(slot.actionPhase || '');
    if (INSTANT_PHASE_TAGS[phase]) return INSTANT_PHASE_TAGS[phase];
    for (const action of slot.actions || []) {
        if (INSTANT_PHASE_TAGS[action]) return INSTANT_PHASE_TAGS[action];
    }
    return INSTANT_PHASE_TAGS.default;
}

function instantEmotionTags(text = '') {
    if (/(?:震惊|惊讶|错愕|shocked|surprised)/i.test(text)) return ['surprised expression'];
    if (/(?:愤怒|恼怒|杀意|angry|furious)/i.test(text)) return ['angry expression'];
    if (/(?:哭|泪|绝望|crying|tears|despair)/i.test(text)) return ['tearful expression'];
    if (/(?:脸红|潮红|害羞|羞耻|blush|embarrassed)/i.test(text)) return ['flushed face', 'intense expression'];
    if (/(?:微笑|大笑|smiling|laughing)/i.test(text)) return ['expressive smile'];
    if (/(?:颤抖|喘息|trembling|panting)/i.test(text)) return ['flushed face', 'trembling body'];
    return ['expressive face'];
}

/**
 * Deterministic emergency prompt builder. It deliberately makes no LLM/API
 * call: the original reply remains the primary prompt author, while this only
 * guarantees a missed female beat still receives an immediate inline button.
 */
export function buildInstantPromptForSlot(slot = {}, previousPrompt = '') {
    const windowText = `${slot.windowText || ''}\n${slot.selectedBeatText || ''}`;
    const scene = SCENE_TAGS.find(([pattern]) => pattern.test(windowText))?.[1] || 'story-accurate environment';
    const phase = String(slot.actionPhase || '');
    const safety = ['erotic_touch', 'manual_stimulation', 'oral_sex', 'penetration', 'thrusting', 'position_change', 'climax', 'ejaculation'].includes(phase)
        ? 'explicit'
        : phase === 'outfit_change' || phase === 'aftercare' ? 'nsfw' : 'safe';
    const tags = [
        ...ANIMA_NATIVE_QUALITY_TAGS,
        safety,
        ...instantParticipantTags(windowText),
        ...instantContinuityTags(previousPrompt),
        ...instantPhaseTags(slot),
        ...instantEmotionTags(windowText),
        scene,
        'anime illustration',
...
export function buildInstantStoryboard(text = '', options = {}) {
    const preferred = Math.max(1, Math.min(6, Number(options.preferred || 3)));
    const maximum = Math.max(preferred, Math.min(6, Number(options.maximum || 6)));
    const plan = options.plan || planStoryboardSlots(text, { preferred, maximum });
    const existing = options.existingPrompts || extractImagePrompts(text);
    const unusedExisting = new Set(existing.map((_, index) => index));
    let continuityPrompt = String(existing[0]?.prompt || options.previousPrompt || '');
    const prompts = plan.slots.map(slot => {
        const ranked = [...unusedExisting].map(index => {
            const record = existing[index];
            const phase = storyActionPhase(record?.prompt || '');
            const actions = storyActionTypes(record?.prompt || '');
            const phaseScore = phase && phase === slot.actionPhase ? 20 : 0;
            const actionScore = actions.filter(action => slot.actions.includes(action)).length * 5;
            return { index, score: phaseScore + actionScore };
        }).sort((a, b) => b.score - a.score || a.index - b.index);
        const match = ranked[0]?.score > 0 ? ranked[0] : null;
        const supplied = match ? existing[match.index]?.prompt : '';
        if (match) unusedExisting.delete(match.index);
        const prompt = supplied ? reinforcePromptLocal(supplied) : buildInstantPromptForSlot(slot, continuityPrompt);
        continuityPrompt = prompt;
        return {
            slot_index: slot.slotIndex,
            event_key: slot.eventKey,
            action_phase: slot.actionPhase,
            after_paragraph_index: slot.paragraphIndexes.at(-1),
            prompt,
            source: supplied ? 'reply' : 'instant-local',
        };
    });
    return { plan, prompts };
}
