...
const NEGATED_FEMALE_STORY_RE = /(?:没有(?:任何)?|并无|不存在|不含|未见|未出现|看不见|周围没有|no|without)\s*.{0,8}?(?:她们?|少女|女孩|女生|女人|女性|女士|姑娘|女王|公主|魔女|女仆|妻子|女友|女人物|\b(?:woman|women|girl|girls|female|lady|ladies|wife|girlfriend)\b)/gi;
...
const ACTION_PATTERNS = [
    ...
    ['kiss', /(?:亲吻|吻住|吻上|接吻|\b(?:kiss(?:es|ed|ing)?)\b)/i],
    ['run', /(?:奔跑|奔向|跑向|飞奔|冲向|扑向|追逐|逃跑|\b(?:run(?:s|ning)?|rush(?:es|ed|ing)?|chase(?:s|d|ing)?|flee(?:s|ing)?)\b)/i],
    ...
    ['position_change', /(?:换了?体位|改变体位|翻身压住|骑到.{0,8}身上|转为.{0,8}体位|\b(?:position chang(?:e|es|ed|ing)|chang(?:e|es|ed|ing) position|switch(?:es|ed|ing)? position|roll(?:s|ed|ing)? (?:her|him) over|mount(?:s|ed|ing)?|cowgirl|riding position|riding (?:the )?(?:adult )?(?:male protagonist|man|him))\b)/i],
...
export function storyParagraphCandidates(text = '') {
    ...
    return paragraphRanges(source).flatMap(item => {
        ...
        if (/^<!--\s*(?:JANIMA_SHOT|JANIMA_STORYBOARD_V2):/i.test(raw)) return [];
...
export function storyBeatGroups(text = '', options = {}) {
    ...
    const analyzed = candidates.map((item, order) => {
        const affirmativeText = item.text.replace(NEGATED_FEMALE_STORY_RE, ' ');
        const directFemale = VISIBLE_FEMALE_STORY_RE.test(affirmativeText);
        const explicitNoFemale = EXPLICIT_NO_FEMALE_STORY_RE.test(item.text);
        if (directFemale) lastFemaleOrder = order;
        else if (explicitNoFemale) lastFemaleOrder = -99;
        ...
        const femaleContext = directFemale || (!explicitNoFemale && (assumeFemale || order - lastFemaleOrder <= 6));
    ...
    for (const beat of eligible) {
        ...
        const bridgeBeats = previousBeat
            ...
            : [];
        const novelDecisiveAction = !sameActionPhase && beat.actions.some(action =>
            action !== 'turn'
            && !group?.actions.includes(action)
            && (ACTION_SALIENCE[action] || 0) >= 4);
...
export function planStoryboardSlots(text = '', options = {}) {
    ...
    if (groups.length >= 4 && decisiveGroups >= 3) targetCount = Math.max(targetCount, 4);
