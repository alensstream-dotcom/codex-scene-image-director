...
const ACTION_PATTERNS = [
    ...
    ['intimacy_change', /(?:进入她|进入他|进入体内|插入|抽插|骑乘|体位|高潮|射精|\b(?:penetrat(?:e|es|ed|ing)|thrust(?:s|ed|ing)?|riding|position change|climax(?:es|ed|ing)?|orgasm(?:s|ed|ing)?)\b)/i],
];
...
export function storyParagraphCandidates(text = '') {
    return paragraphRanges(text).flatMap(item => {
        ...
        if (/^```|<(?:UpdateVariable|initvar|StatusPlaceHolderImpl|imgthink|image|details|head|style|script|iframe|status)\b/i.test(raw)) return [];
...
export function storyActionTypes(text = '') {
    ...
    return ACTION_PATTERNS.filter(([, pattern]) => pattern.test(value)).map(([name]) => name);
}
...
export function storyBeatGroups(text = '') {
    ...
    const analyzed = candidates.map((item, order) => {
        ...
        return { ...item, order, directFemale, femaleContext, actions, transition, emotion, score };
    });
    ...
    for (const beat of eligible) {
        ...
        const sameContinuousAction = Boolean(group && previousBeat
            ...
            && !beat.transition
            && (CONTINUOUS_BEAT_RE.test(beat.text) || sharedAction || (!beat.actions.length && !beat.emotion)));
...
export function planStoryboardSlots(text = '', options = {}) {
    ...
    if (groups.length >= 5 && decisiveGroups >= 5 && groups.filter(group => group.transition).length >= 2) targetCount = Math.max(targetCount, 5);
    if (groups.length >= 6 && decisiveGroups >= 6 && groups.filter(group => group.transition).length >= 4) targetCount = Math.max(targetCount, 6);
