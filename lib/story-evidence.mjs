...
const OUTFIT_STATE_RE = /(?:制服|校服|军装|礼服|连衣裙|裙子|外套|大衣|衬衫|上衣|睡衣|内衣|胸罩|内裤|丝袜|长袜|裤子|短裤|鞋|靴|盔甲|浴巾|浴袍|围裙|比基尼|泳装|衣服|衣着|穿着|穿著|身穿|披着|裹着|戴着|赤裸|裸体|全裸|一丝不挂|\b(?:wearing|dressed in|clad in|uniform|dress|gown|skirt|coat|jacket|shirt|blouse|sweater|pants|trousers|shorts|stockings|socks|shoes|boots|armor|armour|robe|pajamas|lingerie|bra|panties|bikini|swimsuit|nude|naked|topless|bottomless)\b)/i;
...
function quoteProvesCurrentOutfit(packet, member) {
    return Boolean(member?.outfit_change || (member?.outfit && OUTFIT_STATE_RE.test(packet?.quote || '')));
}
    ...
    for (const record of packetRecords) {
        ...
        for (const member of packet?.cast || []) {
            ...
            registry.set(key, clonePacket({
                ...
                ...(quoteProvesCurrentOutfit(packet, member) ? {
                    outfit: member.outfit,
                    outfit_change: member.outfit_change === true,
                } : {}),
            }));
        }
    }
...
export function reconcileShotPacket(packet, registry = new Map()) {
    ...
    next.cast = next.cast.map(member => {
        ...
        if (previous) {
            ...
            if (!quoteProvesCurrentOutfit(next, current)) current.outfit = previous.outfit;
        }
...
export function buildPromptFromShotPacket(packet) {
    ...
    const tags = [
        ...
        'female focus',
        ...style,
        // Qwen/Anima follows early semantic text more reliably. Put the
        // decisive physical action and its critical prop/location before the
        // long per-character DNA blocks, while still repeating every lock.
