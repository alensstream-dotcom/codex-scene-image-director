...
export function validateShotPacket(record, text = '', options = {}) {
    ...
    const quoteStart = packet.quote ? source.lastIndexOf(packet.quote, record.start) : -1;
    if (quoteStart < previousBoundary || quoteStart < 0) {
        issues.push({ code: 'quote_not_in_window', message: '原文证据不在上一张图之后到当前按钮之前' });
    } else if (record.start - (quoteStart + packet.quote.length) > Number(options.maxEvidenceDistance || 2400)) {
    ...
    const quotePhase = storyActionPhase(packet.quote);
    const actionPhase = storyActionPhase(packet.action);
    if (quotePhase !== 'reaction' && actionPhase !== 'reaction' && quotePhase !== actionPhase) {
    ...
    return { ok: issues.length === 0, issues, quoteStart, quotePhase, actionPhase, windowText };
}
...
export function repairStoryboardFromLedger(text = '', options = {}) {
    ...
    for (let index = 0; index < ledger.payload.shots.length; index++) {
        ...
        const issues = [];
        const quoteStart = packet.quote ? source.indexOf(packet.quote, previousQuoteEnd) : -1;
        const quoteEnd = quoteStart < 0 ? -1 : quoteStart + packet.quote.length;
