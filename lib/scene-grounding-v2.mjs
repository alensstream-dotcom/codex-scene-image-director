...
const VISUAL_RE = /(?:走|跑|坐|站|跪|躺|推|拉|抱|搂|吻|抓|握|拔|挥|劈|刺|练|舞|跳|凑|回头|转身|探头|开门|关门|递|喂|抚|摸|按|脱|穿|进入|抽动|高潮|弹琴|抚琴|举剑|出剑|剑招|步法|微笑|流泪|脸红|颤抖|walk|run|sit|stand|kneel|lie|push|pull|embrace|kiss|grab|hold|draw|swing|stab|turn|peek|open|feed|touch|undress|penetrat|climax|play.*instrument)/i;
...
const NSFW_RE = /(?:性交|做爱|性行为|插入|进入她|口交|手交|裸体|全裸|高潮|射精|阴道|阴茎|乳房|乳头|penetrat|intercourse|oral sex|handjob|nude|orgasm|climax|cum)/i;
...
function paragraphsOf(story = '') {
    return String(story || '').split(/\n\s*\n+/).map(value => clean(value, 900)).filter(Boolean);
}
...
function stageFor(text = '') {
    ...
    if (/事后|余韵|aftercare/i.test(text)) return 'aftercare';
    if (/亲吻|接吻|kiss/i.test(text)) return 'kiss';
    if (/拥抱|抱紧|搂住|embrace|hug/i.test(text)) return 'embrace';
    if (/抚琴|弹琴|琴弦|古琴|play.*(?:guqin|zither|instrument)/i.test(text)) return 'music';
...
function peopleFor(text = '', fallback = '1girl') {
...
function styleTags(text = '') {
    if (ANCIENT_RE.test(text)) return ['ancient Chinese fantasy', 'wuxia', 'hanfu', 'traditional Chinese environment'];
...
function actionTags(text = '') {
    ...
    add(/真剑|剑身|剑鞘|拔出.*剑|拔出真剑|draw.*sword/i, 'woman drawing a real Chinese sword from its scabbard, reflective blade');
    ...
    add(/高潮|射精|orgasm|climax|cum/i, 'consensual adult sexual climax, flushed face');
...
function conflictNegative(text = '') {
    const tags = [];
    if (ANCIENT_RE.test(text)) tags.push('western military uniform', 'red military coat', 'epaulettes', 'red cape', 'modern school uniform', 'futuristic armor', 'European palace interior');
...
export function desiredGroundedCount(story = '', { minimum = 3, maximum = 5 } = {}) {
    ...
    const changeCount = (text.match(new RegExp(CHANGE_RE.source, 'gi')) || []).length;
    let count = minimum;
    if (paragraphs.length >= 7 || text.length >= 900 || changeCount >= 2 || INTIMATE_RE.test(text)) count = 4;
...
function makeLocalPacket(quote, window, index) {
    const safety = safetyFor(window);
    return {
        id: `local_${index + 1}_${hashText(quote).toString(36)}`,
        quote,
        people: peopleFor(window),
        cast: [],
        action: englishTags(actionTags(window)).join(', '),
        setting: englishTags(settingTags(window)).join(', '),
        expression: englishTags(expressionTags(window)).join(', '),
        composition: englishTags(compositionTags(window)).join(', '),
        prompt: localPrompt(window),
        negative: uniqueTags(conflictNegative(window)).join(', '),
        stage: stageFor(window),
        safety,
        source: 'local',
    };
}
...
export function buildLocalGroundedPackets(story = '', { maximum = 5 } = {}) {
    ...
    const candidates = paragraphs.map((quote, index, all) => {
        const window = all.slice(Math.max(0, index - 2), index + 1).join(' ');
        const packet = makeLocalPacket(quote, window, index);
        let score = packetScore(packet, index, all.length);
        if (!VISUAL_RE.test(quote) && packet.safety === 'safe') score -= 30;
        if (!(FEMALE_RE.test(window) || storyHasFemale)) score -= 80;
        return { packet, index, score };
    });
    const eligible = candidates.filter(item => item.score > 0);
    const pool = eligible.length >= Math.min(3, paragraphs.length) ? eligible : candidates;
    pool.sort((a, b) => b.score - a.score || b.index - a.index);
    const selected = [];
    const stages = new Set();
    for (const item of pool) {
        const critical = ['nsfw', 'explicit'].includes(item.packet.safety);
        if (!critical && stages.has(item.packet.stage)) continue;
        selected.push(item);
        stages.add(item.packet.stage);
        if (selected.length >= Math.max(1, maximum)) break;
    }
    const targetMinimum = Math.min(3, paragraphs.length, Math.max(1, maximum));
    for (const item of pool) {
...
export function selectGroundedPackets(packets = [], story = '', { minimum = 3, maximum = 5 } = {}) {
    ...
    const selected = [];
    const stages = new Set();
    for (const item of scored.filter(value => ['nsfw', 'explicit'].includes(value.packet.safety))) {
        if (selected.length >= maximum) break;
        if (stages.has(item.packet.stage) && item.packet.stage !== 'penetration') continue;
        selected.push(item);
        stages.add(item.packet.stage);
    }
    for (const item of scored) {
        if (selected.includes(item) || selected.length >= desired) continue;
        if (stages.has(item.packet.stage) && !['nsfw', 'explicit'].includes(item.packet.safety)) continue;
        selected.push(item);
        stages.add(item.packet.stage);
    }
    for (const item of scored) {
    ...
    const order = new Map(paragraphsOf(story).map((paragraph, index) => [clean(paragraph).replace(/\s+/g, ''), index]));
    return selected.slice(0, maximum).sort((a, b) => (order.get(clean(a.packet.quote).replace(/\s+/g, '')) ?? a.index) - (order.get(clean(b.packet.quote).replace(/\s+/g, '')) ?? b.index)).map(item => item.packet);
}
...
export function compileGroundedPrompt(packet = {}, {
    ...
    let sceneTags = promptTags.length >= 5 ? promptTags : uniqueTags([localPrompt(evidence), fallbackTags]);
    if (ANCIENT_RE.test(evidence)) sceneTags = sceneTags.filter(tag => !FORBIDDEN_ANCIENT_POSITIVE_RE.test(tag.trim()));
