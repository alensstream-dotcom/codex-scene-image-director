...
Return exactly ${wanted} grounded candidates when STORY contains that many distinct visible beats; never exceed ${config.maximumShots}. Cover the whole reply: normally the strongest beat from the early, middle, and late portions; fast multi-stage plots may use 4-6. Never front-load all shots. A continuous pose or repeated action is one shot even if described in several paragraphs. Each next shot must depict the most cinematic, emotionally strong, visible instant since the previous selected anchor. Prefer completed physical contact, a decisive result, an emotional climax, or a real outfit/stage change over entering, standing, looking, walking away, scenery, or setup.
...
For each scene return: {"anchor":"an exact 8-80 character verbatim substring copied from STORY or a listed beat","female_names":["names"],"stage":"short unique action/stage key","action_key":"literal English subject-action-object-result translation of only the anchor/window","people":"1girl|1girl, 1boy|2girls","prompt":"literal English location/action/prop translation; no embellishment","outfits":{"character name":"visible outfit at this exact anchor, or CARRY"},"safety":"safe|nsfw|explicit"}. Anchors must occur verbatim in STORY. Never add a location, prop, person, outfit, pose, body contact or result absent from the anchor/window. The plugin validates every concept against verbatim evidence and discards unsupported content.

Physical accuracy is mandatory. Put the exact subject + body part + contact + direction + visible result at the start of action_key and prompt. Distinguish wrist-grabbing from hand-holding, hugging, or merely standing close; distinguish feeding from holding food; distinguish an attempted action from its completed result. Describe where both characters' hands, mouths, bodies, and relevant props are in the decisive frame. Prefer one unambiguous action over a bag of vague mood tags. Use plain English emphasis only: no numeric weights, no artist strings, no quality-word spam. Never soften, replace, or skip a key action just because a nearby pose is easier to draw.`;
...
export function parseDirectorResponse(rawContent, { story, bible = createBible(), settings = {} } = {}) {
    ...
    for (const { scene, index, anchor, position } of anchoredScenes) {
        ...
        const grounding = groundedActionFromEvidence(storyWindow, candidateTranslation);
        ...
        const packet = sanitizePacket({
            ...
            setting: `Only the location, weather, props and clothing present in this verbatim story window: ${storyWindow}`,
...
function finalizeGroundedScenes(scenes, story, bible) {
    ...
    return [...scenes].sort((a, b) => a.position - b.position).map((scene, index) => {
        ...
        previousEnd = end;
        const grounding = groundedActionFromEvidence(evidenceWindow, scene.packet.action);
        ...
        const packet = applyBible(sanitizePacket({
            ...
            cast,
            action: grounding.action,
            setting: `Use only this exact story window; no facts from before or after it: ${evidenceWindow}`,
...
export function completeScenes({ story, parsedScenes = [], bible = createBible(), settings = {} }) {
    ...
    for (const fallback of fallbackScenes(story, bible, config.maximumShots, wanted)) {
