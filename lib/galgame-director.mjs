...
function sceneVisualText(scene) {
    return `${scene?.packet?.stage || ''} ${scene?.packet?.action || ''} ${scene?.packet?.quote || ''}`;
}

function isWeakTransitionValue(value) {
    const text = String(value || '');
    const transition = /开门|打开.{0,8}门|关门|走向玄关|换鞋|穿鞋|拿起书包|收拾.{0,8}(?:包|文件)|匆匆离开|冲向玄关|一前一后|阳光.{0,8}(?:涌入|洒入)|晨光.{0,8}(?:涌入|洒入)|enter(?:ing)?|parting|walking away|open(?:s|ing)? the door|put(?:s|ting)? on shoes|pack(?:s|ing)? (?:a )?bag|sunlight (?:pours|streams) in/i.test(text);
    const nucleus = /拍|摔|砸|重重放|猛地放|抓住|拉住|推开|抢走|拥抱|亲吻|擦掉|递给|喂食|流泪|哭|脸红|颤抖|暴怒|焦急|惊慌|目光.{0,20}(?:来回|锁定)|对峙|攻击|格挡|斩断|高潮|slam|smash|pound|grab|pull|push away|kiss|embrace|wipe|hand(?:ing)?|feed|tears|cry|blush|trembl|furious|anxious|standoff|attack|block|slash|climax/i.test(text);
    return transition && !nucleus;
}

function isWeakTransitionScene(scene) {
    return isWeakTransitionValue(sceneVisualText(scene));
}
...
function cinematicSceneScore(scene) {
    ...
    const value = sceneVisualText(scene);
    let score = 0;
    if (/拍|摔|砸|重重放|猛地放|slam|smash|pound/i.test(value)) score += 8;
    if (/目光.{0,30}(?:来回|移动|扫过)|暴怒.{0,30}焦急|焦急.{0,30}沉默|group[_ -]?(?:reaction|tension)|conflicting emotions|tense tableau/i.test(value)) score += 8;
    if (/抓起.{0,12}书包|从.{0,8}椅子.{0,8}跳|jump(?:s|ing)? (?:up|down|from the chair)|snatch(?:es|ing)? (?:her )?bag/i.test(value)) score += 5;
    ...
    if (isWeakTransitionValue(value)) score -= 12;
    return score;
}

function cinematicSceneCategory(scene) {
    const value = sceneVisualText(scene);
    if (/拍|摔|砸|重重放|猛地放|slam|smash|pound/i.test(value)) return 'object_slam';
    if (/目光.{0,30}(?:来回|移动|扫过)|group[_ -]?(?:reaction|tension)|conflicting emotions|tense tableau/i.test(value)) return 'group_reaction';
    ...
    const stage = String(scene?.packet?.stage || '').toLowerCase().trim().replace(/[^a-z0-9_:-]+/g, '_');
    return stage && !/^(?:story_action|beat_?\d*)$/.test(stage) ? `stage:${stage}` : '';
}

function selectCoveredScenes(scenes, wanted, storyLength) {
    const ordered = [...scenes]
        .sort((a, b) => a.position - b.position)
        .filter(scene => !isWeakTransitionScene(scene) && cinematicSceneScore(scene) >= 0);
    if (ordered.length <= wanted) return ordered;
    const selected = [];
    const usedCategories = new Set();
    const length = Math.max(1, storyLength);
    const strong = ordered.filter(scene => cinematicSceneScore(scene) >= 2);
    const pool = strong.length >= Math.min(wanted, ordered.length) ? strong : ordered;
...
You propose candidates; the plugin chooses final buttons. Scan the WHOLE STORY and return up to ${proposalLimit} distinct high-value female-present beats (at least ${wanted} if available), including alternatives across the real action stages. Never front-load. Repeated paragraphs of one continuous pose are one candidate. Prefer completed contact, decisive result, emotional climax, a multi-character tableau with clearly conflicting visible reactions, or a real stage/outfit change.

Silently apply this visual-focus test to every candidate: (1) what single visible subject makes the shot worth drawing, (2) what indispensable action/contact/expression makes it different from adjacent shots, (3) what exact 0.1-second instant is in progress, and (4) can the image reproduce it using only the local story evidence? Reject packing, taking a bag, routine standing/walking, changing shoes, opening/closing a door, sunlight entering, or leaving as mere transition unless a new female entrance, outfit change, confrontation, or decisive contact happens in that same instant. For dialogue scenes, anchor the strongest visible reaction or group tension, never a destination merely mentioned in speech.
...
function finalizeGroundedScenes(scenes, story, bible) {
    ...
    return [...scenes].sort((a, b) => a.position - b.position).map((scene, index) => {
        ...
        previousEnd = end;
        const actionValue = String(scene.packet.action || '');
        const rendering = actionValue.match(/Grounded English rendering:\s*([\s\S]*?)(?:\.\s+Verbatim story evidence:|$)/i)?.[1]?.trim();
        // Preserve the model's already evidence-checked subject/action/object
        // rendering during final window reconstruction. Dropping it here left
        // only Chinese evidence plus a generic "performs the action" sentence,
        // which made ComfyUI choose an easier unrelated pose.
        const carriedAction = rendering || (/Verbatim story evidence/i.test(actionValue) ? '' : actionValue);
...
export function completeScenes({ story, parsedScenes = [], bible = createBible(), settings = {} }) {
    ...
    const merged = parsedScenes.filter(scene => scene?.packet && cinematicSceneScore(scene) >= 0);
