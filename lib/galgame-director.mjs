...
export function buildDirectorMessages({ story, lastUserMessage = '', bible = {}, settings = {} }) {
    ...
    const proposalLimit = Math.min(12, Math.max(8, wanted * 2));
...
You are proposing a candidate pool, not choosing the final buttons. Scan the complete STORY from beginning to end and return every genuinely distinct high-value female-present visual beat, up to ${proposalLimit} candidates and at least ${wanted} when that many exist. Include strong alternatives from the early, middle, and late portions. Never front-load or stop after finding enough candidates in the first half. The plugin will perform the final global selection. A continuous pose or repeated action is one candidate even if described in several paragraphs. Each candidate must depict one cinematic, emotionally strong, visible instant. Prefer completed physical contact, a decisive result, an emotional climax, or a real outfit/stage change over entering, standing, looking, walking away, scenery, or setup.
